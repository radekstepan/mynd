        ...
        if (this.response.chartType in google.visualization) {
          ...
          if (this.response.pathQuery != null) {
            return google.visualization.events.addListener(chart, "select", function() {
              var description, item, _i, _len, _ref;
              description = '';
              _ref = chart.getSelection();
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                item = _ref[_i];
                if (item.row != null) {
                  description += _this.response.results[item.row + 1][0];
                  if (item.column != null) {
                    description += ' ' + _this.response.results[0][item.column];
                  }
                }
              }
  ...
  */
  ...
  ChartBarView = (function(_super) {
    ...
    ChartBarView.prototype.render = function() {
      $(this.el).html(this.template("chart.bar", {
        "description": this.description,
        "descriptionLimit": this.descriptionLimit
      }));
      return this;
    ...
    ChartBarView.prototype.getPq = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        if (item.row != null) {
          pq = pq.replace("%category", this.response.results[item.row + 1][0]);
          if (item.column != null) {
            pq = pq.replace("%series", translate(this.response, this.response.results[0][item.column]));
          }
          pq = typeof JSON !== "undefined" && JSON !== null ? JSON.parse(pq) : void 0;
          _results.push(this.options.selectCb(pq));
        } else {
